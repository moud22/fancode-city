...
from src.utils import fancode_city, task_completion_rate
...
def test_fancode_users_task_completion(users, todos):
    fancode_users = [user for user in users if fancode_city(user)]
    ...
    for user in fancode_users:
        ...
        completion_percentage = task_completion_rate(user_todos)
...
def test_non_fancode_users_are_ignored(users):
    non_fancode_users = [user for user in users if not fancode_city(user)]
    for user in non_fancode_users:
        assert not fancode_city(user), f"User {user['name']} is incorrectly classified as being from FanCode city"
...
def test_boundary_conditions_for_fancode_city(users):
    ...
    for user in users:
        ...
        if lat in boundary_lat and lng in boundary_lng:
            assert fancode_city(user), f"User {user['name']} at boundary is not correctly classified as FanCode city"
